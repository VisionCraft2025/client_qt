...
- "이번달 에러 통계" → db_find (level: error+UNKNOWN, 7월 범위)
)";
        }
...
- limit 규칙:
  ...
  * "모든", "전체": 100
  * "N개" 명시: N (최대 100)
- 날짜 관련 (현재 날짜: 2025-07-28 기준):
  * "6월": {"timestamp": {"$gte": "2025-06-01T00:00:00Z", "$lt": "2025-07-01T00:00:00Z"}}
  * "7월": {"timestamp": {"$gte": "2025-07-01T00:00:00Z", "$lt": "2025-08-01T00:00:00Z"}}
  * "이번달": {"timestamp": {"$gte": "2025-07-01T00:00:00Z", "$lt": "2025-08-01T00:00:00Z"}}
  * "오늘": {"timestamp": {"$gte": "2025-07-28T00:00:00Z", "$lt": "2025-07-29T00:00:00Z"}}
  * "어제": {"timestamp": {"$gte": "2025-07-27T00:00:00Z", "$lt": "2025-07-28T00:00:00Z"}}
...
- "컨베이어1 오늘 정보 보여줘" → {
    ...
    "query": {
      "device_id": "conveyor_01", 
      "timestamp": {"$gte": "2025-07-28T00:00:00Z", "$lt": "2025-07-29T00:00:00Z"}
    },
    "sort": {"timestamp": -1},
    "limit": 20
  }
- "이번달 에러" 관련 → {
    ...
    "query": {
      ...
      "timestamp": {"$gte": "2025-07-01T00:00:00Z", "$lt": "2025-08-01T00:00:00Z"}
    },
    "sort": {"timestamp": -1},
    "limit": 20
  }
...
날짜 범위 (현재: 2025-07-28 기준):
- "6월": {"$gte": "2025-06-01T00:00:00Z", "$lt": "2025-07-01T00:00:00Z"}
- "이번달": {"$gte": "2025-07-01T00:00:00Z", "$lt": "2025-08-01T00:00:00Z"}
- "오늘": {"$gte": "2025-07-28T00:00:00Z", "$lt": "2025-07-29T00:00:00Z"}
...
예시:
- "피더02 켜줘" → {"topic": "feeder_02/cmd", "command": "on"}
- "컨베이어 2번 꺼줘" → {"topic": "factory/conveyor_02/cmd", "command": "off"}
- "컨베이어 3번 시작" → {"topic": "conveyor_03/cmd", "command": "on"}
- "로봇팔 정지" → {"topic": "robot_arm_01/cmd", "command": "off"}

전체 제어 시:
- "전체 켜줘" → 각 디바이스별로 개별 호출:
  1. {"topic": "feeder_02/cmd", "command": "on"}
  2. {"topic": "factory/conveyor_02/cmd", "command": "on"}
  3. {"topic": "conveyor_03/cmd", "command": "on"}
  4. {"topic": "robot_arm_01/cmd", "command": "on"}

주의: 
- 피더는 2번만 제어 가능 (feeder_01은 제어 불가)
- 컨베이어는 2번, 3번만 제어 가능 (conveyor_01은 제어 불가)
- 로봇팔은 1대만 있음
)";
        }
...
어떤 작업부터 시작해드릴까요?)");
        ...
        if (query.contains("이번달") && query.contains("에러")) {
            return "📊 **이번달 에러 통계 조회**\n\ndb_find 도구를 사용하여 2025년 7월 전체 기간의 에러 로그를 분석하겠습니다.";
        }
        
        return "";  // 특별 케이스가 아니면 빈 문자열 반환
    }
